...
('101','1','2','Molestiae molestiae non doloribus temporibus minima. Eaque sapiente ex assumenda. Eveniet dolorum iure qui mollitia repudiandae vel. Labore quos qui necessitatibus.','1991-05-20 07:06:14'),
('102','2','1','Molestiae molestiae non doloribus temporibus minima. Eaque sapiente ex assumenda. Eveniet dolorum iure qui mollitia repudiandae vel. Labore quos qui necessitatibus.','1991-05-20 07:06:14');
...
INSERT INTO `friend_requests` (`initiator_user_id`, `target_user_id`, `status`, `requested_at`, `confirmed_at`) VALUES ('1', '5', 'approved', '2012-07-18 20:25:26', '2008-06-10 16:46:01');
...
select u_id FROM /* выводим id пользователя */
(
	SELECT u_id, sum(msg_count) as total /* сортируем по количеству сообщений и выбираем одно */
	from (/* ищем все сообщения, либо от пользователя с id1 либо к нему*/
		SELECT to_user_id as u_id,  count(to_user_id) as msg_count FROM messages
		WHERE from_user_id = 1 GROUP BY to_user_id
		HAVING to_user_id != 1
		UNION ALL
		SELECT from_user_id, count(from_user_id) FROM messages
		WHERE  to_user_id = 1 GROUP BY from_user_id
		HAVING from_user_id != 1) as subtable
		WHERE u_id in ( /* фильтруем все сообщения только на друзей пользователя */
...
SELECT gender
FROM (
	SELECT 'Female' as gender, COUNT(*) as count_likes FROM likes WHERE user_id in (
